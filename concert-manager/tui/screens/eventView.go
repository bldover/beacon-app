...
func (v SavedEventViewer) DisplayData() {
	...
	for i := startEvent; i < endEvent; i++ {
		eventData.WriteString(output.FormatEvent((v.events)[i]))
	}
...
func (v *SavedEventViewer) NextScreen(i int) Screen {
	...
	case deleteEvent:
		...
		selectScreen := &Selector[domain.Event]{
			ScreenTitle: "Delete Event",
			Next:        v,
			Options:     v.events[startIdx:endIdx],
			HandleSelect: func(e domain.Event) {
				if err := v.Cache.DeleteSavedEvent(e.ID.Primary); err != nil {
					output.Displayf("Failed to delete event: %v\n", err)
				}
			},
			Formatter: output.FormatEventsShort,
		}
		return selectScreen
		...
		selectScreen := &Selector[string]{
			...
			HandleSelect: func(s string) {
				...
				case searchByArtist:
					...
					v.SearchResultScreen.Events = search.SearchEventsByArtists(name, v.Cache.GetSavedEvents(), search.NoMaxResults, search.LenientTolerance)
				case searchByVenue:
					name := input.PromptAndGetInput("venue name to search", input.NoValidation)
					v.SearchResultScreen.Events = search.SearchEventsByVenue(name, v.Cache.GetSavedEvents(), search.NoMaxResults, search.LenientTolerance)
				default:
					output.Display("Internal error! Check the logs")
					log.Error("Invalid search type selection:", s)
					v.SearchResultScreen.Events = []domain.Event{}
				}
			},
...
func (v *SavedEventViewer) sort() {
	sortFunc := domain.EventSorterDateAsc()
	if v.sortType == dateDesc {
		sortFunc = domain.EventSorterDateDesc()
	}
