...
func (v *DiscoveryViewer) DisplayData() {
	...
	for i := startEvent; i < endEvent; i++ {
		eventData.WriteString(output.FormatEventDetails((v.events)[i]))
	}
	output.Displayln(eventData.String())
}
		...
		selectScreen := &Selector[string]{
			...
			HandleSelect: func(s string) {
				...
				case searchByArtist:
					...
					v.SearchResultScreen.Events = search.SearchEventDetailsByArtist(name, v.events, search.NoMaxResults, search.LenientTolerance)
				case searchByVenue:
					name := input.PromptAndGetInput("venue name to search", input.NoValidation)
					v.SearchResultScreen.Events = search.SearchEventDetailsByVenue(name, v.events, search.NoMaxResults, search.LenientTolerance)
				default:
					output.Display("Internal error! Check the logs")
					log.Error("Invalid search type selection:", s)
					v.SearchResultScreen.Events = []domain.EventDetails{}
				}
			},
...
func (v *DiscoveryViewer) sort() {
	sortFunc := domain.EventDetailsSorterDateAsc()
	if v.sortType == dateDesc {
		sortFunc = domain.EventDetailsSorterDateDesc()
	}
