...
show_usage() {
    echo "Usage: $0 [deploy|start|stop|restart|status|ssh] [environment]"
    echo ""
    echo "Commands:"
...
deploy_service() {
    ...
    if gcloud compute ssh "${SSH_USER}@${INSTANCE_NAME}" --zone "${INSTANCE_ZONE}" --ssh-flag="-oLogLevel=ERROR" -- \
...
show_status() {
    ...
    echo "=== Service Status ==="
    gcloud compute ssh "${SSH_USER}@${INSTANCE_NAME}" --zone "${INSTANCE_ZONE}" --ssh-flag="-oLogLevel=ERROR" -- \
        "sudo systemctl status ${REMOTE_SERVICE_NAME} --no-pager" || echo "Service not found or inactive"
